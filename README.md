...
```bash
cp references/user_profile.example.md references/user_profile.md
# 编辑 references/user_profile.md，填入你的个人信息
```

`user_profile.md` 告诉 AI 你是谁——职业、技术水平、学习目标、兴趣领域。AI 会根据这些信息调整笔记的深度和角度。这个文件越认真填，笔记质量越高。

`references/output_format.md` 定义笔记结构和写作风格，可按需调整。
...
├── SKILL.md                  # Claude Skills 规范入口（agent 使用说明）
├── .env.example              # 环境变量模板
